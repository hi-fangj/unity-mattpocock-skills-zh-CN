...
一份文档由两种内容类型构建——**steps**（agent 执行的有序 actions）和 **reference**（按需查阅的 definitions、rules、facts）——它们可以自由混合：全是 steps（一份 recipe）、全是 reference（review 的 rules、这个 skill），或两者都有。核心决定是每一块内容放在 **information hierarchy** 的哪个位置，这是一架按 agent 多立刻需要该 material 排序的阶梯：
