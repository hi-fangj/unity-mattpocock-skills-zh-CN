...
- **"Does this logic / state model feel right?"** → [LOGIC.md](LOGIC.md)。构建一个自包含的 shareable HTML 文件——free-play buttons 加 tabbed guided walkthroughs——推动 state machine 跑过纸面上难以推理的 cases，并且 non-developer 也能驱动。
...
2. **Trivial to run（运行要零门槛）。** UI prototype 从项目 task runner 里的一个 command 开始——`pnpm <name>`、`python <path>`、`bun <path>` 等；logic demo 是一个用户双击即开的 HTML 文件。无论哪种，启动都不需要思考。对 Unity，使用 documented command、file、scene、prefab、editor menu 或 debug entrypoint；用户不应重建 setup 或 hidden state。
