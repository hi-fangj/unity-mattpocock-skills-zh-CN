...
如果这个 repo 把 external pull requests 当作 request surface（见 issue-tracker 配置），triage 也覆盖它们：**PR 是带附加代码的 issue**，使用相同的 roles、相同的 states、相同的 machine，只有下面标记为 "for a PR" 的几处差异。按 tracker 配置把裸 `#42` 解析为 issue 或 PR。
...
这些是 canonical role names。issue tracker 中实际使用的 label strings 可能不同。映射应该已经提供给你。如果没有，告诉用户运行 `/setup-matt-pocock-skills`。
...
当 PRs 在范围内时，把 external PRs 纳入这些 buckets，并为每行标注 `[PR]` 或 `[issue]`。Discovery 只浮现 *external* PRs（tracker 配置定义了谁算 external），所以 collaborator 正在进行的 PR 不是 triage 工作。这个过滤仅用于 discovery；被明确点名的 PR 无论作者是谁都会被 triage。
...
4. **Grill（如果需要）.** 如果请求需要进一步充实，调用两次 Skill tool，分别运行 "grilling" 和 "domain-modeling"，一轮一轮地把它 grill 成形，在 decisions 落定时打磨 domain terms 并内联更新 `CONTEXT.md`/ADRs。
...
5. **Apply the outcome:**
   ...
   - 对于 `wontfix`，close 该 issue，comment 取决于*原因*：
